...
export class UpdateCategorieComponent implements OnInit {
  ...
  inisialiserProduit(idProduit){
    ...
    this.isLoading = true
    this.http.get(this.userService.baseURL+"/categorie/getById/"+idProduit).subscribe(
      res => {
        let response:any = res
        this.isLoading = false; 
        if(response.status){
          this.categorie = response.resultat
          this.initialiserCentreFormation(this.categorie)
        }else{
  ...
  envoyerRequest(image){
    ...
    this.http.post(this.userService.baseURL+"/categorie/updateCategorie/"+this.idCentre, request, 
    {
      headers: {
